...
function findForum (req, res, next) {
  if (config.multiForum) {
    ...
    if (!id) return next()
    api.forum.findById(id, callback)
  } else {
    api.forum.findDefaultForum(callback)
  }

  function callback (err, forum) {
...
app.post('/topic/create', restrict, findForum, privileges('canChangeTopics'), function (req, res, next) {
